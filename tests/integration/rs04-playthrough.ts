/**
 * RS4: Opening Rounds — Full NATO playthrough.
 * WP airfield strike vs NATO air defenses.
 */
...
function moveToward(state: GameState, fid: string, target: {col:number,row:number}, throttle: 'combat'|'dash'): GameState {
  ...
  if (!ac) {
    // Unknown aircraft: mark as moved to prevent infinite loop
    return { ...state, flights: { ...state.flights, [fid]: { ...f, hasMovedThisPhase: true, hasMoved: true } } };
  }
...
function playRS4() {
  section('RS4: OPENING ROUNDS — FULL PLAYTHROUGH');
  let state = loadScenario('rs04');
          ...
          while (drawSide && rd < 20) {
            ...
            if (drawSide === 'nato') {
              ...
              while (moved < chit.flightsToMove) {
                ...
                const fid = movable[0];
                moved++;
                // Debug: verify hasMovedThisPhase
                if (state.flights[fid].hasMovedThisPhase) { log(`  BUG: ${fid} already moved!`); break; }
                const f = state.flights[fid];
